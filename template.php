		...
		$(document).ready(function() {
			...
			if (window != window.top) {
				if (document.location.hash == '#new') {
					var anchor = $('a[name="new"]');
					if (anchor.length > 0) {
						offset = anchor.offset().top;
					}
				}
				window.parent.jQuery(window.parent).scrollTop(offset);
				window.setInterval(function(){
					var height = $('body').height();
					if (height != lastHeight) {
						window.parent.jQuery('iframe#foro').attr('height', height + 100);
						lastHeight = height;
					}
				}, 1000);
			}
...
function template_body_below()
{
	...
	echo '
		...
		<div id="footer" class="smalltext">
			', theme_copyright(), '<br />Theme by <a href="http://www.dzinerstudio.com">DzinerStudio</a>';
